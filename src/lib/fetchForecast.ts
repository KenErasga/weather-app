import type { OpenWeatherInterval, DayForecast, ForecastData, ForecastError } from "@/types/weather";
...
export function groupByDay(intervals: OpenWeatherInterval[]): DayForecast[] {
    ...
    for (const [date, dayIntervals] of intervalsByDate) {
        ...
        const highTemp = Math.round(Math.max(...dayIntervals.map((interval) => interval.main.temp_max)));
        const lowTemp = Math.round(Math.min(...dayIntervals.map((interval) => interval.main.temp_min)));

        // Prefer the 12:00 forecast for description/icon, fallback to first interval
        const middayInterval = dayIntervals.find((interval) => interval.dt_txt.includes("12:00")) || dayIntervals[0];
...
export async function fetchForecast(city: string): Promise<ForecastData | ForecastError> {
    try {
